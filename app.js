// Mock data (initial defaults, will be overridden by localStorage if available)
const companies = [
...
// Application State - initialize with empty values, will load from localStorage
let communications = []; 
let evaluations = {}; 
...
const STORAGE_KEYS = {
  COMMUNICATIONS: 'confia_communications',
  EVALUATIONS: 'confia_evaluations',
  COUNTER: 'confia_counter'
};
...
function saveDataToStorage() {
  try {
    ...
    localStorage.setItem(STORAGE_KEYS.COUNTER, communicationIdCounter.toString());
...
function loadDataFromStorage() {
  try {
    ...
    if (storedComms) {
      communications = JSON.parse(storedComms);
    ...
    if (storedCounter) {
      communicationIdCounter = parseInt(storedCounter, 10);
    }
    ...
    communications = [];
    evaluations = {};
    communicationIdCounter = 0;
  }
}

// Clear all data from localStorage and reset application state
function clearAllData() {
  if (confirm('ATENÇÃO: Isso irá apagar todas as comunicações e avaliações. Deseja continuar?')) {
    try {
      // Clear localStorage
      ...
      console.log('All data cleared from localStorage and application state');
    } catch (error) {
